#!/usr/bin/env bash
# Train cross-fitted upstream quality models for outer-speech-held-out LAT evaluation.
set -euo pipefail

DATA_ROOT="${DATA_ROOT:-data/experiments/aaai_crossfitted_outer_quality_corrected}"
OUTPUT_PREFIX="${OUTPUT_PREFIX:-aaai_crossfitted_corrected_seed}"
...
for CURRENT_SEED in "${SEEDS[@]}"; do
  ...
  mkdir -p "${seed_root}"
  for outer_dir in "${DATA_ROOT}"/outer_*_speech_*; do
    outer_name="$(basename "${outer_dir}")"
    if [[ -n "${OUTER_FILTER}" && "${outer_name}" != "${OUTER_FILTER}" ]]; then
      continue
    fi
    for data_dir in "${outer_dir}"/inner_* "${outer_dir}"/final_outer; do
      run_model "${data_dir}" "${seed_root}/${outer_name}/$(basename "${data_dir}")"
    done
  done
  echo "Completed cross-fitted quality seed ${CURRENT_SEED}: ${seed_root}"
done
