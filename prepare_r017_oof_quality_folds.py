#!/usr/bin/env python3
"""Prepare leakage-safe quality-model folds for R017 predicted-quality LAT.

Each outer fold holds out one LAT speech group. The quality model trains only
on leakage-safe professional segments from other speeches, selects its
checkpoint on a separate inner-dev speech, and predicts the held-out LAT
segments. This produces out-of-fold LQ/EXP predictions for every LAT row.
"""
...
OUT_DIR = BASE / "data/experiments/r017_oof_quality"
SPLIT_FILES = (
...
def speech_of_safe(row: dict) -> str:
    # Existing safe LQ/EXP files retain this stable format.
...
def choose_inner_dev(outer_speech: str, safe_speeches: list[str]) -> str:
    ...
    return candidates[(safe_speeches.index(outer_speech) + 1) % len(candidates)] if outer_speech in safe_speeches else candidates[0]
...
def main() -> int:
    ...
    if len(lat_rows) != 134:
        raise ValueError(f"Expected 134 feature-complete LAT segments, found {len(lat_rows)}")
    ...
    manifest = {
        "purpose": "R017 out-of-fold professional quality predictions for LAT",
    ...
    for fold_index, outer_speech in enumerate(sorted(by_outer), start=1):
        ...
        test_rows = sorted(by_outer[outer_speech], key=lambda row: row["segment_id"])

        train_keys = {text_key(row) for row in train_rows}
        dev_keys = {text_key(row) for row in dev_rows}
        test_keys = {text_key(row) for row in test_rows}
        overlaps = {
            "train_test_text": len(train_keys & test_keys),
            "dev_test_text": len(dev_keys & test_keys),
            "train_dev_text": len(train_keys & dev_keys),
        }
        if any(overlaps.values()):
            raise ValueError(f"Text overlap in fold {outer_speech}: {overlaps}")
        if any(speech_of_safe(row) == outer_speech for row in train_rows + dev_rows):
            raise ValueError(f"Outer speech leaked into fold {outer_speech}")
        ...
        write_json(fold_dir / "test.json", test_rows)
        manifest["folds"].append({
            ...
            "test_rows_present_in_safe_quality_pool": sum(str(row["segment_id"]) in safe_ids for row in test_rows),
